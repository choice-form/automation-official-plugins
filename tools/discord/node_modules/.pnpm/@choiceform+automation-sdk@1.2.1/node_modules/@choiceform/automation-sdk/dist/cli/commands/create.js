...
export const createCommand = new Command("create")
    ...
    try {
        ...
        const selectedTemplate = TECHNICAL_TEMPLATES.find((t) => t.value === technicalChoice.template);
        const selectedDomain = BUSINESS_DOMAINS.find((d) => d.value === domainChoice.domain);
        const spinner = ora("Creating plugin...").start();
        // 创建插件目录
        const pluginDir = path.resolve(process.cwd(), `${selectedDomain.value}/${basicInfo.name}`);
        if (await fs.pathExists(pluginDir)) {
            spinner.fail(`Directory "${selectedDomain.value}/${basicInfo.name}" already exists`);
            return;
        }
        // 创建目录结构
        ...
        // 移除undefined值
        packageJson.scripts = Object.fromEntries(Object.entries(packageJson.scripts).filter(([_, v]) => v !== undefined));
        ...
        // 生成automation配置
        const automationConfigs = generateAutomationConfigs(basicInfo, selectedTemplate, selectedDomain, displayName, automationNodeType);
        const manifest = {
            name: `@choiceform/${basicInfo.name}`,
            version: "1.0.0",
            description: basicInfo.description,
        ...
        const registry = {
            ...
            security: {
                level: "standard",
                permissions: selectedTemplate.category === "trigger"
                    ? ["network"]
                    : ["network", "data"],
                sandbox: true,
        ...
        // 生成源代码
        const sourceCode = generateSourceCode(basicInfo, selectedTemplate, selectedDomain, displayName);
        ...
        // 生成README.md
        const readmeContent = generateReadme(basicInfo, selectedTemplate, selectedDomain, displayName);
        await fs.writeFile(path.join(pluginDir, "README.md"), readmeContent);
        // 生成测试文件和配置
        if (advancedOptions.includeTests) {
            const testContent = generateTestCode(basicInfo, selectedTemplate, displayName);
            await fs.writeFile(path.join(pluginDir, "tests/index.test.ts"), testContent);
            // 生成 Jest 配置
            ...
            await fs.writeFile(path.join(pluginDir, "tests/__mocks__/@choiceform/automation-sdk.ts"), mockContent);
        }
        // 生成.gitignore
...
Thumbs.db`;
  ...
  <path d="M12 7v10m-5-5h10" stroke="white" stroke-width="2" stroke-linecap="round"/>
        ...
        console.log(chalk.gray(`  📁 Location: ${selectedDomain.value}/${basicInfo.name}`));
        console.log(chalk.gray(`  🔧 Type: ${selectedTemplate.name.split(" - ")[0]}`));
        console.log(chalk.gray(`  📂 Domain: ${selectedDomain.name.split(" (")[0]}`));
        ...
        console.error(chalk.red(`\nFailed to create plugin: ${error instanceof Error ? error.message : error}`));
        process.exit(1);
    }
});
// 生成源代码的函数
function generateSourceCode(basicInfo, template, domain, displayName) {
...
// 生成README的函数
function generateReadme(basicInfo, template, domain, displayName) {
    return `# ${displayName}
...
// 生成测试代码的函数
function generateTestCode(basicInfo, template, displayName) {
    const className = displayName.replace(/\s/g, "");
...
// 生成automation配置的函数
function generateAutomationConfigs(basicInfo, template, domain, displayName, automationNodeType) {
